...
from auditing.models import Option, AuditingCase, Error, PCC_Project, SyncLog
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kw):
        t0 = time()
        log = SyncLog(syncdb_time=NOW())
        # 同步所有的工程案列表
        projects_list = getAllProject()
        ids = []
        ...
        log.pcc_project_num = len(projects_list)
        print time() - t0
        ...
        for n, p in enumerate(PCC_Project.objects.all()):
            ...
            try:
                ...
                for record in auditing_date:
                    ...
                    row.contract_price_change = result['contract_price_change'] * 1000 if result['contract_price_change'] else 0
                    ...
                    for e in errors:
                        if re.search(u'<font size=-1>', e):
                            ...
                            context = '.'.join(context.split('.')[1:]).replace('<fontcolor=red>', '').replace('</font>', '')
                            e = e[seek+14:]
            ...
            print 'done'
        log.auditingcase_num = auditingcase_num
        log.save()
        print time() - t0